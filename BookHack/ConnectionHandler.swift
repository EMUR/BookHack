...
class ConnectionHandler : NSObject,NSFetchedResultsControllerDelegate {
    ...
    private init(maketype:String) {
        ...
        ConnectionHandler.sharedInstance.establishConnection()
    }
    ...
    func addElement(Object: Any) {
        if let bookObj = Object as? Book {
            ...
            ConnectionHandler.sharedInstance.table!.insert(itemToInsert, completion: { (item, error) in
                if error != nil {
    ...
    func getArrayOf(completion: @escaping (_ success: Bool, _ items: [Dictionary<String,Any>]) -> Void) {
        ...
        ConnectionHandler.sharedInstance.table?.read{ (result, error) in
            if let err = error {
                print("ERROR ", err)
